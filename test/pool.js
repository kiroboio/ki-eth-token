...
contract('Pool', async accounts => {
  ...
  it ('should sync pending supply when issuing tokens multiple times', async () => {
    ...
    assert(await pool.validateAcceptTokens(user4, 200, secretHash, rlp.v, rlp.r, rlp.s, { from: manager }), 'invalid signature')
    await pool.executeAcceptTokens(user4, 200, Buffer.from(secret), rlp.v, rlp.r, rlp.s, { from: manager} )
    assert.equal(initPendingSupply, +(await pool.supply({ from: manager })).pending)
  })
  ...
  it ('should sync pending supply when issuing tokens to multiple accounts', async () => {
    ...
    const messageHash = web3.utils.sha3(message)
    const rlp = await web3.eth.accounts.sign(messageHash.slice(2), getPrivateKey(user4))
    assert(await pool.validateAcceptTokens(user4, 300, secretHash, rlp.v, rlp.r, rlp.s, { from: manager }), 'invalid signature')
    await pool.executeAcceptTokens(user4, 300, Buffer.from(secret), rlp.v, rlp.r, rlp.s, { from: manager} )
    assert.equal(initPendingSupply + 400, +(await pool.supply({ from: manager })).pending)
  ...
  it ('supply & accounts info should be accurate after extensive use of the system', async () => {
    ...
    const user3InitAccount = await pool.account(user1)
    const user4InitAccount = await pool.account(user1)
    const user5InitAccount = await pool.account(user1)
    const user1InitTokens = +await token.balanceOf(user1)
    const user2InitTokens = +await token.balanceOf(user1)
    const user3InitTokens = +await token.balanceOf(user1)
    const user4InitTokens = +await token.balanceOf(user1)
    const user5InitTokens = +await token.balanceOf(user1)
    ...
    const user2Account = await pool.account(user1)
    const user3Account = await pool.account(user1)
    const user4Account = await pool.account(user1)
    const user5Account = await pool.account(user1)
    const user1Tokens = +await token.balanceOf(user1)
    const user2Tokens = +await token.balanceOf(user1)
    const user3Tokens = +await token.balanceOf(user1)
    const user4Tokens = +await token.balanceOf(user1)
    const user5Tokens = +await token.balanceOf(user1)
    ...
    assert.equal(+user2Account.balance, +user2InitAccount.balance) 
    ...
    assert.equal(+user3Account.balance, +user3InitAccount.balance) 
    ...
    assert.equal(+user4Account.balance, +user4InitAccount.balance) 
    assert.equal(+user4Account.pending, +user4InitAccount.pending) 
