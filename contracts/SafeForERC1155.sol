...
//0x5978e4630233612269aC217784A41fB3EA720d30
        ...
        payable external
    {
        ...
        bytes32 id = keccak256(abi.encode(token, msg.sender, to, tokenId, value, tokenData, fees, secretHash));
        require(s_erc1155Transfers[id] == 0, "SafeTransfer: request exist"); 
        s_erc1155Transfers[id] = 0xffffffffffffffff;
        emit ERC1155TransferDeposited(token, msg.sender, to, tokenId, value, fees, secretHash);
    }
    ...
    function collectERC1155(
        ...
        onlyActivator()
    {
        bytes32 id = keccak256(abi.encode(token, from, to, tokenId, value, tokenData, fees, secretHash));
        ...
        payable external
    {
        require(msg.value == fees, "SafeTransfer: msg.value must match fees");
        require(to != msg.sender, "SafeTransfer: sender==recipient");
        bytes32 id = keccak256(abi.encode(token, msg.sender, to, tokenIds, values, tokenData, fees, secretHash));
        require(s_erc1155Transfers[id] == 0, "SafeTransfer: request exist"); 
        s_erc1155Transfers[id] = 0xffffffffffffffff;
    ...
    function collectBatchERC1155(
        ...
        onlyActivator()
    {
        bytes32 id = keccak256(abi.encode(info.token, from, to, info.tokenIds, info.values, info.tokenData, info.fees, info.secretHash));
        uint256 tr = s_erc1155Transfers[id];
        require(tr > 0, "SafeTransfer: request not exist");
        require(uint64(tr) > now, "SafeTranfer: expired");
        require(uint64(tr>>64) <= now, "SafeTranfer: not available yet");
        ...
        IERC1155(info.token).safeBatchTransferFrom(from, to, info.tokenIds, info.values, info.tokenData);
        emit ERC1155BatchCollected(info.token, from, to, info.tokenIds, info.values);
    } 


    // ----------------------- swap - single ERC1155 <--> ETH -------------------------------------------
        ...
        external payable 
    {
        ...
        if (info.token0 == address(0)) {
            ...
            IERC1155(info.token0).safeTransferFrom(
                from,
                msg.sender,
                info.tokenId0,
                info.value0,
        ...
        if (info.token1 == address(0)) {
            ...
            IERC1155(info.token1).safeTransferFrom(
                msg.sender,
                from,
                info.tokenId1,
                info.value1,
    ...
    function swapRetrieveETHToERC1155(address to, SwapETHToBatchERC1155Info memory info) external {
        bytes32 id = keccak256(
            abi.encode(
                msg.sender,
                to,
